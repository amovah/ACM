...
#  Write a function answer(xs) that takes a list of integers representing the power output levels of
#  each panel in an array, and returns the maximum product of some non-empty subset of those numbers.
...
#  giving the product 2*(-3)*(-5) = 30.  So answer([2,-3,1,0,-5]) will be "30".
#
...
#  1000 (some panels are malfunctioning so badly that they're draining energy,
#  but you know a trick with the panels' wave stabilizer that lets you combine two negative-output
...
def answer(list):
    ...
    if len(negative) % 2 != 0:
        del negative[negative.index(max(negative))]
